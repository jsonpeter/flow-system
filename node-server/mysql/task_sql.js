...
module.exports = {
    select_dayNewNum(){
        ...
        let str='select  a.storeId,count(*) as new_number  from (SELECT *  FROM  users_log  where  date(dateTime)=date(now()) group by faceId) as a,' +
            ' (SELECT faceId,dateTime FROM users_info) as b where a.faceId=b.faceId and  date(a.dateTime)=date(b.dateTime)  group by a.storeId'
        console.log(str)
        return str;
    },
    select_dayAllNum(){
        //各个门店的总人数汇总
        let str='select storeId ,count(*) as "number" from (SELECT *  FROM  users_log  where  date(dateTime)=date(now()) group by faceId ,storeId ) as s  group by s.storeId  HAVING COUNT(*) > 0';
        console.log(str)
        return str;
    },
